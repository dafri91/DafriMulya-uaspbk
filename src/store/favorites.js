...
export const useFavoritesStore = defineStore("favorites", {
  ...
  getters: {
    isFavorite: (state) => (productId) =>
      state.favorites.some((fav) => fav.productId === productId),
  ...
  actions: {
    async fetchFavorites() {
      ...
      try {
        ...
        if (snapshot.exists()) {
          this.favorites = Object.entries(snapshot.val()).map(([id, val]) => ({
            id,
            ...val,
          }));
        } else {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          const match = entries.find(
            ([id, val]) => val.productId === productId
          );
          if (match) {
            await remove(
              ref(db, `favorites/${authStore.user.uid}/${match[0]}`)
            );
          }
        }
        await this.fetchFavorites();
