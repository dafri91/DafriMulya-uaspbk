...
import { useRequireAuth } from "../composables/useRequireAuth"; // import helper
...
const { checkAuth } = useRequireAuth(); // gunakan helper
...
const addToCart = async (event) => {
  ...
  await cartStore.addToCart(props.product, 1); // tambahkan await dan quantity
};
...
<template>
  <div @click="viewProduct"
    ...
    <div class="flex-1 flex flex-col justify-between p-3 sm:p-4">
      <!-- Brand & Rating -->
      <div class="flex justify-between items-center text-xs text-neutral-500 mb-1">
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 mt-auto">
        <div class="flex flex-col min-w-0 truncate">
          ...
          <span v-if="discountPercentage > 0" class="text-xs text-neutral-400 line-through truncate">
            {{ formatPrice(product.originalPrice) }}
          </span>
        </div>
        <button v-if="!authStore.isAdmin" @click.stop="addToCart"
