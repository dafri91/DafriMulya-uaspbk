...
const fetchReviews = async (id) => {
  try {
    const response = await axios.get(`http://localhost:3001/reviews?productId=${id}`)
...
const loadProductDetail = async (id) => {
  ...
  quantity.value = 1
  await productsStore.fetchProduct(parseInt(id))
  await productsStore.fetchProducts() // Needed for recommendations
...
// Watch for route changes (e.g. when clicking a recommended product)
watch(
  () => route.params.id,
  (newId) => {
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="!product" class="text-center py-12">
      ...
      <button
        ...
        class="bg-primary text-white px-6 py-3 rounded-lg hover:bg-primary/90 transition duration-150"
      >
        Back to Products
      </button>
    </div>
...
<nav class="flex mb-8" aria-label="Breadcrumb">
  <ol class="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <span class="text-gray-400 mx-2">/</span>
      <span class="text-gray-900 truncate max-w-[12rem] sm:max-w-[16rem] md:max-w-[24rem]">
        {{ product.name }}
      </span>
    </li>
  </ol>
        ...
        <div>
          ...
          <div v-if="product.images && product.images.length > 1" class="flex space-x-2">
            <button
              ...
              :class="[
                'w-20 h-20 rounded-lg overflow-hidden border-2 transition duration-150',
          ...
          <div v-if="product.specifications" class="mb-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
              <dl class="grid grid-cols-1 gap-3">
                ...
                >
                  <dt class="font-medium text-gray-900 capitalize">{{ key.replace('_', ' ') }}:</dt>
...
<div class="mb-6">
  <div class="flex items-center space-x-4 mb-4">
    <div class="flex items-center space-x-2">
      ...
      <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden">
        <button
          ...
          class="px-3 py-2 text-gray-700 hover:bg-gray-100 transition"
        >−</button>
    ...
    <!-- Stock Status -->
    <div class="flex items-center">
      <div :class="['w-3 h-3 rounded-full mr-2', product.inStock ? 'bg-green-500' : 'bg-red-500']"></div>
  ...
  <div class="flex space-x-4">
    <button
      ...
      class="flex-1 bg-primary text-white py-3 px-6 rounded-lg font-semibold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition duration-150"
    >
      Add to Cart
    </button>
            ...
            <div class="flex items-start justify-between">
              <div>
                <div class="flex items-center mb-2">
                  ...
                  <span class="ml-2 font-semibold text-gray-900">{{ review.userName }}</span>
                </div>
