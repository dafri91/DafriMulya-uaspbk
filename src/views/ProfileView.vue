        ...
        <aside class="lg:col-span-1">
          <div class="bg-white rounded-xl shadow-sm p-6 sticky top-24">
            <div class="flex items-center space-x-4 mb-6">
              <div
                class="w-16 h-16 bg-primary/20 rounded-full flex items-center justify-center text-primary text-2xl font-bold">
            ...
            <nav class="space-y-2">
              <button @click="activeTab = 'overview'"
                :class="['w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 flex items-center gap-3', activeTab === 'overview' ? 'bg-primary/10 text-primary font-semibold' : 'text-neutral-600 hover:bg-neutral-100']">
              ...
              <router-link to="/orders"
                class="w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 flex items-center gap-3 text-neutral-600 hover:bg-neutral-100">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <button @click="activeTab = 'settings'; isEditing = false"
                :class="['w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 flex items-center gap-3', activeTab === 'settings' ? 'bg-primary/10 text-primary font-semibold' : 'text-neutral-600 hover:bg-neutral-100']">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="pt-2 border-t">
                <button @click="handleLogout"
                  class="w-full text-left px-4 py-2 rounded-lg text-red-600 hover:bg-red-50/50 transition-colors duration-200 flex items-center gap-3">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <main class="lg:col-span-3">
          <div class="bg-white rounded-xl shadow-sm p-8">
            <div v-if="activeTab === 'overview'">
              ...
              <p class="text-neutral-600 mb-6">Welcome back! Here you can see your basic account information and recent
                activity.</p>
              ...
              <div v-if="!isEditing" class="space-y-6">
                ...
                <div class="border-t pt-6">
                  <button @click="isEditing = true"
                    class="bg-primary text-white px-5 py-2 rounded-lg font-semibold hover:bg-primary/90 transition">Edit
                    Profile</button>
                </div>
              </div>
              ...
              <form v-else @submit.prevent="handleUpdateProfile" class="space-y-6">
                ...
                <div>
                  ...
                  <textarea v-model="form.address" id="address" rows="3"
                    class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"></textarea>
                </div>
                <div class="flex justify-end space-x-4 border-t pt-6">
                  <button type="button" @click="resetForm"
                    class="bg-white px-5 py-2 border border-neutral-300 rounded-lg text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition">Cancel</button>
                  <button type="submit"
                    class="bg-primary text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-primary/90 transition">Save
                    Changes</button>
                </div>
              </form>
            </div>
